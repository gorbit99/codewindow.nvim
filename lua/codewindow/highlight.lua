...
function M.setup()
  ...
  vim.api.nvim_set_hl(
    0,
    "CodewindowError",
    { link = "DiagnosticSignError", default = true }
  )
  vim.api.nvim_set_hl(
    0,
    "CodewindowAddition",
    { fg = "#aadb56", default = true }
  )
  vim.api.nvim_set_hl(
    0,
    "CodewindowDeletion",
    { fg = "#fc4c4c", default = true }
  )
  vim.api.nvim_set_hl(
    0,
    "CodewindowUnderline",
    { underline = true, sp = "#ffffff", default = true }
  )
end
...
function M.extract_highlighting(buffer, lines)
  ...
  buf_highlighter.tree:for_each_tree(function(tstree, tree)
    ...
    local iter = query
      :query()
      :iter_captures(root, buf_highlighter.bufnr, 0, line_count + 1)
      ...
      if hl then
        ...
        if c ~= nil then
          local start_row, start_col, end_row, end_col =
            ts_utils.get_vim_range({ ts_utils.get_node_range(node) }, buffer)
          ...
          for y = start_row, end_row do
            for x = start_col, math.min(end_col, minimap_char_width) do
              ...
              highlights[minimap_y][minimap_x][c] = (
                highlights[minimap_y][minimap_x][c] or 0
              ) + 1
            end
          end
        end
